...
OPENAI_VISION_TIMEOUT_MS=20000
OPENAI_DIAGRAM_MODEL=gpt-5.5
OPENAI_DIAGRAM_TIMEOUT_MS=12000
VISION_MAX_TOKENS=1800
```

如果要临时切回 Kimi，可在 Render 环境变量里改为：
...
DeepSeek 文本接口不负责识别题目图片。当前项目预留独立视觉模型接口，默认使用 OpenAI 视觉接口；也可以通过 `VISION_PROVIDER=kimi` 切换到 Kimi / Moonshot，或通过 `VISION_PROVIDER=zhipu` 切回智谱接口。

配置视觉模型后，用户可以上传题目图片，也可以直接在对话框里 Ctrl+V 粘贴题目截图。网页会在后台自动把图片转写成题目文字，不把识别文字塞进输入框，而是直接交给智能体开始启发式提问。

右侧“解题结构图”会优先使用 OpenAI 生成结构图方案，再由网页用 SVG 精确渲染为节点关系图。几何题不会再用固定三角形/圆形模板伪画原题图，避免把不存在的点线角关系画出来。

手机端上传图片时，前端会先把图片压缩到适合识别的大小，避免微信、手机相册里的大图导致识别请求失败。

如果未配置 `OPENAI_API_KEY`，上传入口仍会显示，但会提示当前网站还没有配置图片识别模型，用户可以手动输入题目文字。
...
网站已内置 `personalized-math-learning-implementation v2.0.0` 方法包。覆盖小学一年级至高中三年级，并提供家庭、学生、教师三条服务路径：

- 家庭诊断：依据错题、讲题转写或练习作品收集证据，再按学段安排最小任务。
- 学生自主学习：现有“开始知识点学习”入口会按小学低段、小学中高段、初中或高中调用对应任务库。
- 教师课堂改造：依据教案、任务单、学生作品或课堂片段生成可执行的课堂调整建议。
- 实践反馈：根据下一轮真实作品判断保持、升级或退阶，不用模型猜测代替证据。
- 成长档案：保存诊断与实践记录，便于连续观察学生变化。

实现资源位于 `implementation/`，运行时由 `implementation-retriever.js` 按服务意图选择性注入；普通解题不会加载整套资料。成长记录接口为 `GET /api/growth-profile`，需要当前登录身份。

任务库分为小学一至二年级、小学三至六年级、初中和高中四套，并使用十维诊断、提示层级、升级/保持/退阶规则。大学场景仍可使用网站原有数学讲解能力，但不属于本落地包已校准范围。
